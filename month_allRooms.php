...
//si $room est 0 alors on va montrer toutes les rooms dans le calendrier
...
//ici $room sera 0 si pas spécifié dans l'URL
...
echo make_room_select_html('month.php', $area, 0, $year, $month, $day);
...
echo "<h2>" . utf8_strftime($strftime_format['monthyear'], $month_start)
  . " - " . htmlspecialchars("$this_area_name") . "</h2>\n";
echo "</div>\n";
...
//Get all meetings for this month for all rooms
// row[0] = Start time
// row[1] = End time
// row[2] = Entry ID
...
for ($day_num = 1; $day_num<=$days_in_month; $day_num++)
{
  $sql = "SELECT start_time, end_time, id, name, type,
                 repeat_id, room_id, status, create_by
            FROM $tbl_entry
             WHERE start_time <= $pm7[$day_num] AND end_time > $am7[$day_num]
        ORDER BY start_time";
    ...
    for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
    {
      ...
      else
      {
        ...
        $d[$day_num]["shortdescrip"][] = htmlspecialchars($row['name']);
		 // Récupération du nom de la salles pour l'afficher plus tard
		 $room = $row['room_id'];
		 //echo(sql_query1("SELECT room_name FROM mrbs_room WHERE id=$room"));
		 $d[$day_num]["roomname"][] = sql_query1("SELECT room_name FROM $tbl_room WHERE id=$room");
		 //$d[$day_num]["roomname"][] = $room;
      }
    ...
    if (isset($d[$cday]["id"][0]))
    {
      ...
      for ($i = 0; $i < $n; $i++)
      {
        ...
        $description_text = utf8_substr($d[$cday]["shortdescrip"][$i], 0, 255);
		 $salle = $d[$cday]["roomname"][$i];;
        $full_text = $slot_text . " " . $description_text . "-" .$salle;
