...
function getAllEntryForToday() {
  ...
  if ($num_table != 0) {
	 ...
	 while ($row = mysql_fetch_array($result_table)) {
		...
		$_xml .="<reservation id=\"$row[name]\" salle=\"$row[room_id]\" debut=\"$debut\" fin=\"$fin\" />";
	 } 
...
function getAllEntryForTodayForRoom ($roomId) {
	  //tout d'abord on se connecte à la base de donnée
	  $database="cd922";
	  $connect = mysql_connect("localhost","cd922","jk6xkw5k");
	  mysql_select_db($database, $connect);
	  ...
	  $timestamp_soir = mktime(22,0,0, $month, $day, $year);
		  ...
		  //Ensuite on envoie une requete SQL pour sélectionner une table dont on veux les informations
	  $sql = "SELECT	  room_id, description, status, name, start_time, end_time
				  FROM mrbs_entry
	  ...
	  if ($num_table != 0) {
		 ...
		 while ($row = mysql_fetch_array($result_table)) {
			...
			$_xml .="<reservation id=\"$row[name]\" debut=\"$debut\" fin=\"$fin\" />\r\n";
		 } 
